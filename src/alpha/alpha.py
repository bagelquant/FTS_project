...
def decay_linear(df, period=10):
    ...
    return pd.DataFrame(na_lwma, index=df.index, columns=['CLOSE'])  # type: ignore
# endregion
...
class Alphas(object):
    def __init__(self, df_data):
        ...
        self.returns = df_data['pct_change']
...
#                             columns=self.close.columns)
    ...
    def alpha023(self):
        ...
        alpha = pd.DataFrame(np.zeros_like(self.close),index=self.close.index,columns=['close'])  # type: ignore
